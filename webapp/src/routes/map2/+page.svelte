...
<Card class="text-center" size="xl">
    ...
    <form action="./map" method="get" class="grid gap-4 items-end md:grid-cols-10" data-sveltekit-noscroll
        ...
        <Button color="green" class="col-span-2" on:click={pasteCoordinates}>
            <Icon name="map-pin-outline"/> <p class="pl-2 text-xs">Ctrl + V</p>
        </Button>
        ...
        <Button color="primary" class="{!setPossible ? 'hidden' :''}" type="submit">
            <Icon name="map-pin-outline"/> <p class="pl-2 text-xs">Show</p>
        </Button>
        <Button color="primary" class="{!sharePossible ? 'hidden' :''}" on:click={copyMapUrlToClipboard}>
            <Icon name="share-nodes-outline"/> <p class="pl-2 text-xs">Share</p>
        </Button>

        <Button outline color="red" on:click={clearMap}>
            <Icon name="close-solid"/> <p class="pl-2 text-xs">Clear</p>
        </Button>
    </form>
...
<Card class="text-center hidden" size="xl">
    <div role="img" class="m-auto p-0 block" id="map_area"
        ...
        <div class:hidden={mouse == null}
            ...
            <div class="absolute pointer-events-none block w-[10px] h-[10px] rotate-45 bg-inherit border-inherit border-t border-l left-16 -ml-[5px]" style=" top: -6px;"></div>
        </div>
...
<Map {points} {showTeleports} {showPointOfInterest}></Map>
<style lang="scss">
  :global(.marker_img) {
