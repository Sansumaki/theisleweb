<script lang="ts">
    ...
    function LocationToLatLng(location: TeleportLocation | PoiLocation): LatLngExpression {
        return [Math.round(location.Location_X / 1000), Math.round(location.Location_Y / 1000)];
    }

    function CorrectedLatLng(location: TeleportLocation | PoiLocation): LatLngExpression {
...
<div class="w-full h-screen absolute left-0 top-0 mapDiv">
    <Leaflet bounds={_mapBounds} {showTeleports} {showPointOfInterest}>
        {#if _teleportLocations.length > 0}
            {#each _teleportLocations as location, index}
                <Marker latLng={CorrectedLatLng(location)} width={40} height={40} isTeleport>
        ...
        {#if _poiLocations.length > 0}
            {#each _poiLocations as location, index}
                <Marker latLng={CorrectedLatLng(location)} width={40} height={40} isPoi>
