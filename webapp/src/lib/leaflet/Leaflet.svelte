<script lang="ts">
	...
	import L, { ImageOverlay, type LatLngBoundsExpression, type LatLngExpression, Map } from 'leaflet';
	...
	const { setShowTeleportLocations, setShowPoiLocations }: {
		setShowTeleportLocations: (show: boolean) => void,
		setShowPoiLocations: (show: boolean) => void
	} = getContext('showFilters');
	...
	onMount(() => {
		...
		let baseLayers: { [key: string]: ImageOverlay } = {};
		mapSources.forEach(m => {
			baseLayers[m.name] = L.imageOverlay(m.url, m.bounds as LatLngBoundsExpression);
		});
		...
		map = L.map(mapElement, {
			...
			zoomSnap: 0,
			zoomDelta: 0.5,
			maxBounds: maxBounds as LatLngBoundsExpression,
		...
		bounds = bounds ?? maxBounds as LatLngBoundsExpression;

		let Position = L.Control.extend({
			latLng: undefined as (HTMLElement | undefined),
			options: {
				position: 'bottomright'
			},

			onAdd: function() {
				this.latLng = L.DomUtil.create('div', 'mouseposition rounded p-1 px-2');
				return this.latLng;
			},

			updateHTML: function(lat: number, lng: number) {
				if (this.latLng) {
					this.latLng.innerHTML = 'Latitude: ' + lng + '   Longitiude: ' + lat; // converted to isle coordinates
				}
			}
		});
		...
		for (let i = expandedBounds[0][0] * 5; i <= expandedBounds[1][0] * 5; i += gridSize) {
			...
			L.polygon(p as LatLngExpression[], { color: 'black', weight: (i == 0 ? 0.5 : 0.2), opacity: 0.5 }).addTo(map);
		}
	...
	$: if (map && showTeleports != undefined && teleportLayer != undefined) {
		if (showTeleports) {
			map.addLayer(teleportLayer);
		} else {
			map.removeLayer(teleportLayer);
		}
		setShowTeleportLocations(showTeleports);
	}

	$: if (map && showPointOfInterest != undefined && poiLayer != undefined) {
		if (showPointOfInterest) {
			map.addLayer(poiLayer);
		} else {
			map.removeLayer(poiLayer);
		}
		setShowPoiLocations(showPointOfInterest);
	}

</script>

<div class="leaflet-bar leaflet-control" bind:this={menuElement}>
	<button on:click={() => showTeleports = !showTeleports } title="Telepsort Locations"
		 class="rounded-t border-b border-black z-[800] p-1 bg-[#749053] hover:bg-[#aad07b] w-[30px] h-[30px] m-0 float-left clear-both">
		<svg viewBox="0 0 512 512">
			...
			{:else}
				<path fill="#000"
							...
							style="" transform="matrix(1, 0, 0, 1, 0, -1.4210854715202004e-14)"></path>
				<line
					style="fill: rgb(216, 216, 216); stroke: rgb(0, 0, 0); stroke-width: 24px; stroke-linecap: round; transform-origin: 253.693px 271.447px;"
					x1="110.252" y1="124.533" x2="397.135" y2="418.361"
					transform="matrix(-1, 0, 0, -1, 0.000061035156, 0)"></line>
			{/if}
		</svg>
	</button>
	<button on:click={() => showPointOfInterest = ! showPointOfInterest} title="Points of Interest"
	class="rounded-b z-[800] p-1 bg-[#749053] hover:bg-[#aad07b] w-[30px] h-[30px] m-0 float-left clear-both">
